...
import { editPassword, reset } from "../../actions/auth";
...
function Password() {
	...
	useEffect(() => {
		if (Object.keys(errors).length == 0 && !checkAreInputsEmpty(form)) {
			dispatch(
				editPassword(
					{ password: form.oldPassword, newPassword: form.newPassword },
					user.result._id
				)
			);
		}
		...
		<form className='password' noValidate>
			...
			<div className='password__box'>
				<input
					...
					required
				/>
				<label htmlFor='oldPassword' className='password__label'>
					Stare hasło
				</label>
				<span
					className='password__icons'
					onClick={() => showPassword(oldPass, oldPassShow, oldPassHide)}>
				...
				<p
					className={
						errors.oldPassword
							? "password__error-text password__show-input-error"
			...
			<div className='password__box'>
				<input
					...
					required
				/>
				<label htmlFor='newPassword' className='password__label'>
					Nowe hasło
				</label>
				<span
					className='password__icons'
					onClick={() => showPassword(newPass, newPassShow, newPassHide)}>
				...
				<p
					...
					{errors.newPassword ? errors.newPassword : "error"}
				</p>
			</div>
			<div className='password__box'>
				<input
					...
					required
				/>
				<label htmlFor='repeatPassword' className='password__label'>
					Powtórz hasło
				</label>
				<span
					className='password__icons'
					onClick={() =>
						showPassword(repeatPass, repeatPassShow, repeatPassHide)
					}>
				...
				<p
					...
					{errors.repeatPassword ? errors.repeatPassword : "error"}
				</p>
			</div>
			<p
				className={
					success
						? "password__submit-message success"
						: "password__submit-message"
				}>
				{!success && authData}
				{success && authData}
			</p>
