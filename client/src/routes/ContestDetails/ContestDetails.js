...
function ContestDetails() {
	...
	const { contest, messageError, success } = useSelector(
				...
				<>
					<div className='contest-details__box'>
						<div className='contest-details__box-item'>
							...
							<p className='contest-details__text'>
								{new Date(contest.startContest).getDate()} -{" "}
								{new Date(contest.endContest).getDate()}{" "}
								{new Date(contest.startContest).toLocaleString("default", {
									month: "long",
								})}
								, {new Date(contest.startContest).getFullYear()}
							</p>
						</div>
						<div className='contest-details__box-item'>
							<h3 className='contest-details__title'>Miejsce</h3>
							<p className='contest-details__text'>
								{contest.typeContest === "on-line"
									? `zawody ${contest.typeContest}`
									: contest.place}
							</p>
						</div>
						<div className='contest-details__box-item'>
							...
							<p className='contest-details__text'>
								{new Date(contest.startRegistration).getDate()}{" "}
								{new Date(contest.startRegistration).toLocaleString("default", {
									month: "long",
								})}{" "}
								{new Date(contest.startRegistration).getFullYear()},{" "}
								{new Date(contest.startRegistration).getHours() - 1}:
								{new Date(contest.startRegistration).getMinutes()}
							</p>
						</div>
						<div className='contest-details__box-item'>
							<h3 className='contest-details__title'>Koniec rejestracji</h3>
							<p className='contest-details__text'>
								{new Date(contest.endRegistration).getDate()}{" "}
								{new Date(contest.endRegistration).toLocaleString("default", {
									month: "long",
								})}{" "}
								{new Date(contest.endRegistration).getFullYear()},{" "}
								{new Date(contest.endRegistration).getHours() - 1}:
								{new Date(contest.endRegistration).getMinutes()}
							</p>
						</div>
						<div className='contest-details__box-item'>
							<h3 className='contest-details__title'>Konkurencje</h3>
							{contest.events?.map((event, index) => (
								<span className='contest-details__text' key={index}>
									{event.label}
									{index < contest.events.length - 1 && ","}{" "}
								</span>
							))}
						</div>
						<div className='contest-details__box-item'>
					...
					<div className='contest-details__change-view-btns'>
						<button
							className='contest-details__change-view-btn'
							ref={firstButtonRef}
							onClick={e => changeViewAndMarker(e, "register")}>
							Rejestracja
						</button>
						<button
							className='contest-details__change-view-btn'
							onClick={e => changeViewAndMarker(e, "users")}>
							Zawodnicy
						</button>
						<button
							className='contest-details__change-view-btn'
							onClick={e => changeViewAndMarker(e, "live")}>
							Wyniki live
						</button>
					...
					{changeView === "users" && (
						<ContestUsers users={contest.users} events={contest.events} />
					)}
